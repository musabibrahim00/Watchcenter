/* ═══════════════════════════════════════════════════
   DONE TODAY — Structured expandable archive
   ═══════════════════════════════════════════════════ */
...
function DoneTodaySection({
  ...
  return (
    <div className="flex flex-col mt-3">
      ...
      <button
        onClick={onToggle}
        className="flex items-center gap-3 px-3 py-2.5 -mx-1 rounded-lg hover:bg-[rgba(255,255,255,0.015)] transition-colors duration-200 cursor-pointer group"
      >
        <motion.div
          animate={{ rotate: isOpen ? 90 : 0 }}
          transition={{ duration: 0.25, ease: "easeInOut" }}
          className="text-[#4a5568] group-hover:text-[#62707D] transition-colors duration-200"
        >
          <ChevronRight size={14} />
        </motion.div>
        <span className="font-['Inter',sans-serif] text-[13px] text-[#89949e] font-medium group-hover:text-[#a0a9b2] transition-colors duration-200">
          Done Today ({missions.length})
        </span>
        {missions.length > 0 && (
          <div className="flex-1 flex justify-end">
            <span className="font-['Inter',sans-serif] text-[11px] text-[#2c3c47] group-hover:text-[#3d4d5a] transition-colors duration-200">
              {manualCount > 0 && autoCount > 0
                ? "Manual approvals \u00B7 Auto-resolved"
                : manualCount > 0
                ? "Manual approvals"
                : autoCount > 0
                ? "Auto-resolved"
      ...
      {!isOpen && (
        <div className="pl-9 pb-1">
          <span className="font-['Inter',sans-serif] text-[11px] text-[#3d4d5a] leading-4">
            {summaryText}
          </span>
        </div>
      )}
